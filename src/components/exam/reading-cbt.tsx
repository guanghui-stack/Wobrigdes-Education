 ...
 * nền trắng, font Arial, 2 panel kéo giãn, thanh part dưới cùng,
 * đánh dấu câu, highlight/ghi chú văn bản, đồng hồ đếm ngược.
 */
...
/* CSS Custom Highlight API — chưa có trong type lib cũ nên khai báo tối giản */
...
export function ReadingCbtExam({
    ...
    parts.forEach((p, partIdx) =>
      p.questionGroups.forEach((g) =>
        g.questions.forEach((q) =>
          out.push({ qid: q.id, number: out.length + 1, partIdx, type: g.type })
        )
      )
    );
    return out;
  ...
  const numberOf = useMemo(() => {
    ...
    flat.forEach((f) => m.set(f.qid, f.number));
    return m;
  }, [flat]);
  ...
  const partRange = useMemo(
    () =>
      parts.map((_, i) => {
        const qs = flat.filter((f) => f.partIdx === i);
        return { start: qs[0]?.number ?? 0, end: qs[qs.length - 1]?.number ?? 0, count: qs.length };
      }),
    [parts, flat]
  );
  ...
  const [split, setSplit] = useState(52); // % bề rộng panel bài đọc
  ...
  const answeredIn = (i: number) =>
    flat.filter((f) => f.partIdx === i && (answers[f.qid] ?? "").trim()).length;
  ...
  let renderedNo = 0; // đánh số hiển thị khi duyệt tuần tự
  ...
  return (
    ...
    >
      {/* ===== Thanh trên: tiêu đề + đồng hồ + nộp bài ===== */}
      ...
      <div className="shrink-0 border-b border-neutral-300 bg-[#f2f2f2] px-4 py-2">
        ...
        <p className="text-sm text-neutral-700">
          Read the text and answer questions {partRange[part]?.start}–
          {partRange[part]?.end}.
        </p>
      </div>
      ...
      >
          ...
          {parts.map((p, i) => (
            <div key={i} hidden={i !== part}>
              ...
              {p.passage.paragraphs.map((para, pi) => (
                <p key={pi} className="mb-4 text-[15px] leading-[1.7]">
                  {para}
                </p>
              ))}
            </div>
        ...
        >
          {parts.map((p, i) => {
            ...
            let no = partRange[i].start - 1;
            return (
              <div key={i}>
                {p.questionGroups.map((g, gi) => {
                  const groupStart = no + 1;
                  const groupEnd = no + g.questions.length;
                  return (
                    <div key={gi} className="mb-8">
                      <p className="text-[15px] font-bold">
                        Questions {groupStart}–{groupEnd}
                      </p>
                      <p className="mb-4 mt-1 whitespace-pre-line text-[15px] leading-relaxed text-neutral-800">
                        {g.instruction}
                      </p>
                      {g.questions.map((q) => {
                        no += 1;
                        ...
                        return (
                          <div key={q.id} id={`q-box-${q.id}`} className="mb-6">
                            <div className="flex items-start gap-2">
                              <span
                                className={`flex h-7 min-w-7 shrink-0 items-center justify-center border text-sm font-bold ${
                                  isActive
                                    ? "border-neutral-800"
                                    : "border-transparent"
                                }`}
                              ...
                              <p className="flex-1 pt-0.5 text-[15px] leading-relaxed">
                                {q.prompt}
                              </p>
                              <button
                                type="button"
                            ...
                            {g.type === "GAP" ? (
                              <input
                                ...
                                aria-label={`Câu ${n}`}
                                className="ml-9 mt-2 w-full max-w-xs border border-neutral-400 px-3 py-1.5 text-[15px] focus:border-black focus:outline-none"
                              />
                            ) : (
                              <div className="ml-9 mt-2 space-y-1.5">
                                {(q.options ?? []).map((opt) => {
                                  const value =
                                    g.type === "MC" ? opt.trim().charAt(0) : opt;
                                  return (
                                    <label
                                      key={opt}
              ...
              <ul className="space-y-3">
                {notes.map((nt) => (
                  <li key={nt.id} className="border border-neutral-200 p-2">
                    ...
                    {nt.note && (
                      <p className="mt-1 text-sm leading-snug">{nt.note}</p>
                    )}
                    <button
                      type="button"
        ...
        <div className="flex min-w-0 flex-1 items-stretch overflow-x-auto">
          {parts.map((_, i) =>
            i === part ? (
              <div
                key={i}
                className="flex shrink-0 items-center gap-0.5 bg-neutral-100 px-3 py-1.5"
              >
                ...
                {flat
                  ...
                  .map((f) => {
                    const isAnswered = (answers[f.qid] ?? "").trim() !== "";
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => jumpTo(f)}
                        aria-label={`Câu ${f.number}${isAnswered ? " (đã trả lời)" : ""}${isMarked ? " (đã đánh dấu)" : ""}`}
